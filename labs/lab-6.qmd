...
The data set is called `births14` and it is available in your `data` folder.
...
b.  In 2-3 sentences, explain how the regression line to model these data is fit, i.e., based on what criteria Python determines the regression line.
...
a.  Create a new variable `transformed_parsim` that is calculated as `math.log(parsim/(1-parsim))`. Add this variable to your data frame.
...
Save each model output as `dt_model`, `dist_model`, `BM_model`, and `prec_model`, respectively.
...
If $r\approx0$ then $R^2\approx0$, indicating the model is a very bad fit for the data.

You can calculate $R^2$ using the `.rsquared` function.
For example, you can calculate $R^2$ for `dt_model` using the code `dt_model.rsquared`.
...
The data set is called `gapminder`, and it is available in your `data` folder.
